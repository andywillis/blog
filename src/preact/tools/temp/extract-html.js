...
function List({
  ...
  if (variant === 'unordered') {
    return y("ul", null, items.map(item => y(ListItem, {
      key: item.id,
      html: item.html
    })));
  }
  return y("ol", null, items.map(item => y(ListItem, {
    key: item.id,
    html: item.html
  })));
}
...
function Home({
  ...
  return y(k, null, y(Header, null), y(Entries, {
    entries: entries
  }), y(Footer, null));
}
